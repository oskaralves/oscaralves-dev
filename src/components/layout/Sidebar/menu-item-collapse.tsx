  ...
  return (
    <li className="group relative flex flex-col">
      <>
        <button
          onClick={() => toggleOpen()}
          className={`mx-1 flex flex-row items-center justify-between space-x-2 rounded-md p-3 transition-colors hover:bg-neutral-500/10 ${isOpen ? 'rounded-b-none bg-neutral-500/10' : ''} ${
        ...
        >
          ...
          <div
            className={`flex transition-all ${isOpen ? 'rotate-180' : ''} ${!sidebarExpanded ? 'absolute left-[13px] top-8' : ''}`}
          >
            <ChevronDownIcon
        ...
        {isOpen && item?.children?.length && (
          <ul
            className={`mx-1 flex flex-col py-2 ${sidebarExpanded ? 'p-1 py-2' : ''} ${isOpen ? 'rounded-b-md bg-neutral-500/10' : ''}`}
          >
